...
fs::dir_exists(fs::path(repicentre_repo_path, "data", "clean") ) 
fs::dir_exists(fs::path(repicentre_repo_path, "data", "raw"))

# fetch only the simulated LL from the clean data folder in simulated_data repository 
clean_ll <- fs::dir_ls(fs::path(data_repo_path, "data", "clean"), regexp = ".*/simulated_measles[^/]*\\.rds")

# fetch only the simulated LL from the clean data folder in simulated_data repository 
raw_ll <- fs::dir_ls(fs::path(data_repo_path, "data", "final"))
...
for(i in clean_ll){
  ...
  fs::file_copy(
    path = i, 
    new_path = fs::path(repicentre_repo_path, "data", "clean"), 
    overwrite = TRUE
                )
}
...
for(i in raw_ll){
  ...
  fs::file_copy(
    path = i, 
    new_path = fs::path(repicentre_repo_path, "data", "raw"), 
    overwrite = TRUE
  )
